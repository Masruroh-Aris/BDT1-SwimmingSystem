...
<head>
    ...
    <title>Certificate - {{ $data->athlete_name }}</title>
    {{-- QRCode JS --}}
    {{-- QRCode JS via CDN for reliability --}}
    ...
    <style>
        ...
        .certificate-container {
            ...
            background-image: url('{{ asset($certificate->image_path) }}');
...
<body>
    ...
    <div class="certificate-container">
        ...
        <div class="overlay-text cert-number" 
             style="{{ $layout ? 'top: '.$layout['cert_number']['top'].'; left: '.$layout['cert_number']['left'].'; position: absolute;' : '' }}">
            {{ str_pad($data->id, 4, '0', STR_PAD_LEFT) }}/SERTIF/{{ date('Y') }}
        </div>

        <!-- 2. Athlete Name -->
        <div class="overlay-text athlete-name"
             style="{{ $layout ? 'top: '.$layout['athlete_name']['top'].'; left: '.$layout['athlete_name']['left'].'; transform: none; text-align: center; width: auto; position: absolute;' : '' }}">
            {{ $data->athlete_name }}
        </div>

        <!-- 3. Role / Rank (Under "SEBAGAI") -->
        <div class="overlay-text role-info"
             style="{{ $layout ? 'top: '.$layout['role_info']['top'].'; left: '.$layout['role_info']['left'].'; transform: none; text-align: center; width: auto; position: absolute;' : '' }}">
            @if(isset($data->rank))
                JUARA {{ $data->rank }}
            @else
                PARTICIPANT
        ...
        <div class="overlay-text event-info"
             style="{{ $layout ? 'top: '.$layout['event_info']['top'].'; left: '.$layout['event_info']['left'].'; transform: none; text-align: center; width: auto; position: absolute;' : '' }}">
             {{ $data->event_name }}
        </div>

        <!-- 5. QR Code Area -->
        <div class="qr-code-box"
             style="{{ $layout ? 'top: '.$layout['qr_code']['top'].'; left: '.$layout['qr_code']['left'].'; bottom: auto; position: absolute;' : '' }}">
             <!-- Using Public API for reliable image generation -->
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data={{ urlencode(route('certificate.show.registration', $data->id)) }}" 
                 alt="QR Code" 
