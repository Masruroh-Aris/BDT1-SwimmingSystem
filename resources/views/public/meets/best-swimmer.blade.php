...
<head>
    ...
    <title>Best Swimmer Performance</title>
    ...
    <script>
        async function exportToPDF() {
            ...
            try {
                ...
                pdf.save('Best_Swimmer_Performance.pdf');
            } catch (error) {
                ...
                <table class="w-full border-collapse min-w-[800px] text-xs">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $index % 2 == 0 ? '' : 'bg-gray-50' }} hover:bg-gray-100">
                            {{-- Rank Calculated Automatically ($index + 1) --}}
                            <td class="border border-gray-300 px-2 py-2 text-center font-semibold">{{ $index + 1 }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-left">{{ $swimmer->athlete_name }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center">{{ $swimmer->sex }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center">{{ $swimmer->ag }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-left">{{ $swimmer->team_name }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center">{{ $swimmer->gold }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center">{{ $swimmer->silver }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center">{{ $swimmer->bronze }}</td>
                            <td class="border border-gray-300 px-2 py-2 text-center font-bold">{{ $swimmer->point }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="9" class="border border-gray-300 px-2 py-4 text-center text-gray-500">No results found for this meet</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
